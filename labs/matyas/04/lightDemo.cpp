//
// AVT Phong shading with pointlight and spotligth: you should press key ´l' to toggle between source light types
...
void renderScene(void) {
	...
	loc = glGetUniformLocation(shader.getProgramIndex(), "spotDir");
	float spotdir[4] = { -lightPos[0], -lightPos[1], -lightPos[2], 0.0f };   //to origin
	multMatrixPoint(VIEW, spotdir, res);  //Transform the spotdir to eye coordinates
	glUniform4fv(loc, 1, res);
	loc = glGetUniformLocation(shader.getProgramIndex(), "spotCosCutOff");
	glUniform1f(loc, 0.98f);
...
void processKeys(unsigned char key, int xx, int yy)
{
	switch(key) {
		...
		case 'l':  //Toggle between pointlight and spotligth
			if (!spotlight_mode) {
				spotlight_mode = true;
...
void init()
{
	...
	// create geometry and VAO of the cube for the floor
	objId = 4;

	float amb2[] = { 0.0f, 0.3f, 0.0f, 1.0f };
	float diff2[] = { 0.1f, 0.8f, 0.1f, 1.0f };
	...
	memcpy(mesh[objId].mat.emissive, emissive, 4 * sizeof(float));
	mesh[objId].mat.shininess = 20.0f;
	mesh[objId].mat.texCount = texcount;
	createCube();
