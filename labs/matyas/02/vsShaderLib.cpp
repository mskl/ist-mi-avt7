...
void VSShaderLib::loadShaderFromString(VSShaderLib::ShaderType st, const char* s)
{
    if (s != NULL) {
        // const char *ss = s;

        pShader[st] = glCreateShader(spGLShaderTypes[st]);
        glShaderSource(pShader[st], 1, &s, NULL);
...
void VSShaderLib::loadShader(VSShaderLib::ShaderType st, std::string fileName) {
    ...
    std::cout << "Reading text file " << fileName << ": " << s << "\n" << std::endl;
    std::cout << "s != NULL: " << (s != NULL) << "\n" << std::endl;
